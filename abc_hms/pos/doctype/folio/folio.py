...
class Folio(Document):
    ...
    def folio_invoice_list(self, docstatus: Optional[int] = None):
        ...
        return frappe.get_all("POS Invoice", filters)

    def folio_active_invoice(self):
        return frappe.db.get_value("POS Invoice", {"folio": self.name, "docstatus": 0})
    ...
    def folio_item_transfer(
        ...
        active_invoice_doc.pos_invoice_item_transfer(
    ...
    @frappe.whitelist()
    def folio_merge(
        self, destination_folio: str, destination_window: str, keep_source: bool = False
    ):
    ...
    def make_payment(self, mode_of_payment: str, amount: float, window: str):
        try:
            ...
            if not active_invoice:
                frappe.throw(
                    "No active POS Invoice found for this folio.",
                    exc=frappe.ValidationError,
                )
                return
            active_invoice.append(
                "payments",
                {
    ...
    def before_submit(self):
        ...
        if reservation:
            ...
            if not (
                reservation_dict["departure"] == business_date
                and reservation_dict["reservation_status"] == "In House"
            ):
                frappe.throw(
                    f"Folio cann't be submitted unless the linked reservation departure date is today"
                )
        ...
        #             f"folio_blanace_issue:{json_dumps(balance_result)}"
        #         )
        invoices = frappe.get_all(
            "POS Invoice", {"folio": self.name, "docstatus": 0}, pluck="name"
        )
    ...
    def after_insert(self):
        try:
            ...
            frappe.log_error(
                ...
                f"Folio after_insert: Failed to create Folio Window for {self.name}",
            )
            raise e
