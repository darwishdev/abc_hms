...
class POSSessionRepo:
    def pos_session_find(self, pos_sesison: str):
        def sql_call(cur, _):
            ...
            return {"item_details": item_details, "payment_details": payment_details}

        return run_sql(sql_call)

    def pos_session_upsert(
        self, docdata: POSSession, commit: bool = True
    ) -> POSSession:
        try:
    ...
    def pos_session_list(
        self, filters: Optional[POSSession]
    ) -> Optional[List[POSSession]]:
