...
class POSInvoiceRepo:
    ...
    def pos_invoice_find_for_date(
        ...
        try:
            ...
            invoices = frappe.get_all(
                "POS Invoice", filters={"for_date": for_date}, fields=fields_to_fetch
            )
    ...
    def pos_invoice_item_update_widnow(self, names: List[str], folio_window: str):
    ...
    def pos_invoice_item_transfer(self, payload: PosInvoiceItemTransferRequest):
        try:
            ...
            if not source_invoice:
                raise NotFound(
                    f"the source invoice not found {payload['source_invoice']}"
                )
            if "destination_invoice" in payload:
                dest_invoice = frappe.get_doc(
                    "POS Invoice", payload["destination_invoice"]
                )

            if "destination_folio" in payload:
                pos_profile = frappe.db.get_value(
                    "POS Invoice", payload["source_invoice"], "pos_profile"
                )
                ...
                dest_invoices = frappe.get_all(
                    "POS Invoice",
                    {"folio": payload["destination_folio"], "for_date": for_date},
                    pluck="name",
                )
                if len(dest_invoices) == 0:
                    raise frappe.NotFound(
                        "Destination Folio Don not Have POS Invoce For Current Business Date"
                    )
                dest_invoice = frappe.get_doc("POS Invoice", dest_invoices[0])
            if not dest_invoice:
                raise NotFound(
                    f"the source invoice not found {payload['destination_invoice']}"
                )
            items_passed = "items" in payload
            for item in source_invoice.items:
                if item.folio_window == payload["source_window"] and (
                    not items_passed or item.name in payload["items"]
                ):
            ...
            if not items_passed:
                frappe.db.sql(
                    "update `tabFolio Window` fw SET fw.folio = %s where name = %s",
    ...
    def pos_invoice_update(self, doc, docdata, commit):
        ...
        if items is not None:
            for row in items:
                ...
                if row_name:
                    existing_row = next(
                        (i for i in doc.items if i.name == row_name), None
                    )
        ...
        if payments is not None:
            for row in payments:
                ...
                if row_name:
                    existing_row = next(
                        (p for p in doc.payments if p.name == row_name), None
                    )
                    if existing_row:
                        setattr(existing_row, "pos_session", frappe.local.pos_session)
                        for field, val in row.items():
        ...
        required_fields = ["grand_total", "base_grand_total", "total", "net_total"]
        doc_dict = doc.as_dict()
        # doc.set_total_in_words()
        for f in required_fields:
            if doc_dict[f] is None:
                frappe.throw(f"[MISSING FIELD] {f} is None in POSInvoiceData")
        doc.save()
    ...
    def pos_invoice_upsert(
        self,
        docdata: POSInvoiceData,
        reset_items: bool = True,
        reset_payments: bool = True,
        commit: bool = True,
    ) -> POSInvoiceData:
            ...
            if docdata["is_return"]:
                ...
                if not against:
                    raise frappe.ValidationError(
                        "against invoice is required on return onvpoce"
                    )
