...
CREATE TABLE IF NOT EXISTS inventory_lookup (
    id INT AUTO_INCREMENT PRIMARY KEY,
    lookup_type VARCHAR(50) NOT NULL,   -- e.g. 'room status', 'physical status'
...
CREATE TABLE IF NOT EXISTS
  `inventory_log` (
    ...
    `ooo_status` int(1),
    `ooo_reason` TEXT,
    `physical_status` int(1) ,
    `service_status` int(1)
...
CREATE TABLE IF NOT EXISTS
  `inventory` (
    ...
    `persons` int(1),
    `hk_status` int(1) , -- {0 : vancant , 1 : occ},
    `room_status` int(1) , -- {0 : dirty , 1 : clean , 2 : inspected},
    `ooo_status` int(1) , -- {0 : in order, 1 : ooo , 2 : ooc },
    `ooo_reason` TEXT, -- {0 : in order, 1 : ooo , 2 : ooc },
    `physical_status` int(1) , -- {0 : vacant , 1 : occ , 2 },
    `service_status` int(1) , -- {0 : no_status , 1 : dnd  , 2 : make up},
...
CREATE OR REPLACE TABLE room_type_rates (
