...
class PropertySetting(Document):
    def validate_arrivals(self):
        arrivals = frappe.db.sql(
            """
            SELECT r.name , r.reservation_status from tabReservation r WHERE r.reservation_status =
            'Arrival' AND r.docstatus != 2
        """,
            as_dict=True,
        )
        if len(arrivals) > 0:
            frappe.throw(
                f"Arrivals found for current business date {json.dumps(arrivals)}",
    ...
    def validate_departures(self):
        ...
        if len(departures) > 0:
            frappe.throw(
                f"Departures  found for current business date  {json.dumps(departures)}",
                exc=frappe.ValidationError,
            )
            raise frappe.ValidationError(f"Arrivals Found for data {for_date}")
    ...
    def validate_sessions(self, for_date: str):
        ...
        if len(sessions) > 0:
            frappe.throw(
                f"Session Found for data {json.dumps(sessions)}",
                exc=frappe.ValidationError,
            )
            raise frappe.ValidationError(
                f"Sessions  found for current business date {for_date}"
            )

    def validate(self):
        original_value = frappe.db.get_value(
            "Property Setting", self.name, "business_date"
        )
