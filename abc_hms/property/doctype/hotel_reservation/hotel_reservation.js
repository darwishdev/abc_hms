frappe.listview_settings["Hotel Reservation"] = {
    onload(listview) {
        ...
        listview.filter_area.add([["Hotel Reservation", "check_in_completed", "=", "0"]]);
    },
};
frappe.ui.form.on("Hotel Reservation", {
    ...
    room: function (frm) {
        console.log("frm2", frm.doc.room);
        if (frm.doc.room) {
            console.log("frm", frm.doc.room);
    ...
    refresh(frm) {
        ...
        if (showCheckIn) {
            frm.add_custom_button("Check In", function () {
                frappe.call({
                    method: "abc_hms.check_in", // adjust path if needed
                    args: {
...
async function fetch_and_render_availability(frm) {
  ...
  `,
    ...
    // 5) Call GET endpoint
    frappe.call({
        method: "abc_hms.get_availability",
        type: "GET",
        args: {
            start: startInt,
            end: endInt,
