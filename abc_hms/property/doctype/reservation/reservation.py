...
class Reservation(Document):
    ...
    def handle_sync(self , is_submit: bool):
        frappe.publish_progress(10, title="Saving Reservation", description="Check If Critical Fields Has Been Changed : Statred")
        # raise AvailabilityError("Reservation update blocked: has errors2")
        if not is_submit:
            is_critical_effect = self.critical_fields_check()
            time.sleep(.2)
            frappe.publish_progress(20, title="Saving Reservation", description="Check If Critical Fields Has Been Changed : Done")
            if not is_critical_effect:
                time.sleep(.02)
                frappe.publish_progress(100, title="Saving Reservation", description="No Critical Fields Changed And Reservation Updated")
                return
    ...
    def reservation_date_sync(self):
        ...
        frappe.publish_progress(50, title="Saving Reservation", description="Reservation Dates Sync Started")
        ...
        try:
            ...
            frappe.publish_realtime(
                event='handle_error',
                message={'message':err_msg,'type':'Availability' , "frm":filtered_form},
            ...
            frappe.publish_realtime(
                event='handle_error',
                message={'message':err_msg,'type':'Room Sharing', "frm":filtered_form},
                user=frappe.session.user  # or specific user
            )
            raise e
        finally:
    ...
    def reservation_folio_sync(self):
        if not frappe.db.exists("Folio", {"reservation": self.name}):
            ...
            folio.save()
            frappe.publish_progress(100, title="Saving Reservation", description="Reservation Dates Synced Successflly")
