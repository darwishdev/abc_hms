...
def check_in(reservation_id):
    ...
    try:
        ...
        if reservation.room:
            frappe.db.set_value(
                "Room", reservation.room, "room_status", "Occupied"
            )
...
def get_dashboard_data(as_of_date):
    ...
    total_rooms,  available, out_of_order = frappe.db.sql(
        """
        SELECT
    ...
    # --- In House ---
    in_house = frappe.db.count(
        "Hotel Reservation",
        {
            "check_in_completed": 1,
            "check_out_completed": 0,
            "check_in_date": ("<=", as_of_date_str),
            "check_out_date": (">=", as_of_date_str),
            "docstatus": 1
        },
    )
    # --- Arrivals / Departures ---
