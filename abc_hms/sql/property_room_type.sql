...
IF p_date_from > p_date_to THEN SIGNAL SQLSTATE '45000'
SET
  MESSAGE_TEXT = 'Date From must be earlier than Date To And Both Must Be 8 DIGITS';
...
SET
  p_room_categories = NULLIF(TRIM(p_room_categories), '');
...
WITH
  ...
  rooms AS (
    ...
    FROM
      ...
      and rt.pay_master is false
      join `tabRoom Category` rc on rt.room_category = rc.name
    WHERE
      rc.property_name = p_property
      AND (
        p_room_categories IS NULL
        OR FIND_IN_SET(rc.name, p_room_categories)
      )
      AND (
  ...
  ooo_rooms as (
    select
      r.room_type,
      count(r.name) total_ooo
    from
      room_date rd
      join dates d on rd.for_date = d.for_date
      join tabRoom r on r.name = rd.room
    WHERE
      rd.out_of_order_status > 0
    GROUP BY
      r.room_type
  ),
  reserved_rooms as (
    select
      rd.room_type,
      count(rd.for_date) total_reserved
    from
      reservation_date rd
      join dates d on rd.for_date = d.for_date
    GROUP BY
  ...
  resp as (
    SELECT
      ...
      (
        r.total_rooms - COALESCE(rr.total_reserved, 0) - COALESCE(ooo.total_ooo, 0)
      ) AS available
    FROM
      rooms r
