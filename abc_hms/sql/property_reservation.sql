...
CREATE OR REPLACE PROCEDURE `reservation_inouse_invoices` (IN p_current_date INT) BEGIN
WITH
  reservations AS (
    SELECT
  ...
  room_item AS (
    SELECT
      r.reservation,
      IF(
        (r.rate_code_rate - SUM(i.amount)) > 0,
        r.rate_code_rate - SUM(i.amount),
        0
  ...
  prepared_room_item as (
    ...
    from
      room_item r
      join room_discounts d on r.reservation = d.reservation
  ),
  items AS (
    SELECT
      NULL income_account,
      ...
      IF(
        ...
        JSON_ARRAYAGG(
          JSON_OBJECT(
            ...
            0,
            'folio_window',
            CONCAT(r.folio, '-W-001')
          )
        )
      ) items
    FROM
      reservations r
      LEFT JOIN items i ON r.reservation = i.reservation
    GROUP BY
      r.reservation,
      r.customer,
      r.rate_code,
